...
namespace Track2
{
    internal class LogicAppTests : Track2TestBase
    {
        ...
        public async Task GlobalSetUp()
        {
            ...
            string rgName = "LogicAppRG-Custom-2000";
        ...
        public async Task P0_Workflow_E2E()
        {
            ...
            string filepath = Directory.GetCurrentDirectory() + @"/TestData/definition.json";
            ...
            // get
            var workflow = await collection.GetAsync(workflowName);
        ...
        public async Task P1_IntegrationAccount_Maps()
        {
            ...
            {
                Content = BinaryData.FromString(Xslt30MapContent),
        ...
        [Test]
        public async Task Create_LogicApp_Connect_EventHubs()
        {
            var collection = _resourceGroup.GetLogicWorkflows();

            // Create event hub
            var eventHubNamespace = await _resourceGroup.GetEventHubsNamespaces().CreateOrUpdateAsync(WaitUntil.Completed,"conneventhubnamespace1216",new EventHubsNamespaceData(_resourceGroup.Data.Location));
            var eventhub = await eventHubNamespace.Value.GetEventHubs().CreateOrUpdateAsync(WaitUntil.Completed,"evnethubforconn1",new EventHubData());
            var ruleData = new EventHubsAuthorizationRuleData()
            {
                Rights =
                {
                    Azure.ResourceManager.EventHubs.Models.EventHubsAccessRight.Manage,
                    Azure.ResourceManager.EventHubs.Models.EventHubsAccessRight.Send,
                    Azure.ResourceManager.EventHubs.Models.EventHubsAccessRight.Listen,
                }
            };
            var authorizationRule = await eventhub.Value.GetEventHubAuthorizationRules().CreateOrUpdateAsync(WaitUntil.Completed, "mamagerule", ruleData);
            var keys = await authorizationRule.Value.GetKeysAsync();
            //keys.Value.PrimaryKey;

            // Create - takes 30 seconds
            // Create - takes 30 seconds
            string workflowName = "workflow0000";
            string filepath = Directory.GetCurrentDirectory() + @"/TestData/LogicAppDefinitionWithConnEventhub.json";
            byte[] definition = File.ReadAllBytes(filepath);
            LogicWorkflowData data = new LogicWorkflowData(_resourceGroup.Data.Location)
            {
                Definition = new BinaryData(definition),
