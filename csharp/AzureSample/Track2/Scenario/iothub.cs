...
namespace Track2
{
    internal class iothub : Track2TestBase
    {
        ...
        [Test]
        public async Task RunActionTest()
        {
            ResourceGroupResource resourceGroup = await GetResourceGroup("LogicAppRG-2697");
            LogicWorkflowResource lapp = resourceGroup.GetLogicWorkflow("workflow2219");

            LogicWorkflowRunResource laRun = lapp.GetLogicWorkflowRun("08585326104775083883302537231CU07");

            LogicWorkflowRunActionResource ac1 = laRun.GetLogicWorkflowRunAction("Compose");
            LogicWorkflowRunActionResource ac2 = laRun.GetLogicWorkflowRunAction("Send_an_email_(V2)");

            Console.WriteLine(ac1.Data.OutputsLink.Uri.ToString());
            Console.WriteLine(ac2.Data.OutputsLink.Uri.ToString());
        }
    }
}
